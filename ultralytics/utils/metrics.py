...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, RIoU=False, LARIoU = False, eps=1e-7):
    """
    ...
    iou = inter / union
    if CIoU or DIoU or GIoU or LARIoU or RIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or RIoU or LARIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if LARIoU:
                ...
                ch2 = ch ** 2 + eps
                return iou - (
                        rho2 / c2 + torch.log(1 + rho_w2) / torch.log(1 + cw2) + torch.log(1 + rho_h2) / torch.log(
                    1 + ch2))

            if RIoU:
                rho2 = (((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (
                        b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4) ** 1  # center dist ** 2
                ...
                iou = inner_iou(box1, box2, xywh=xywh, ratio=0.7)
                ...
                # ratio_pred = (w1 + eps) / (h1 + eps)
                # ratio_gt = w2 / h2
                # v = (ratio_pred - ratio_gt).pow(2)
                # alpha = torch.sigmoid(5 * iou)
                # size_penalty = torch.log(1 + torch.sqrt(w2 * h2))
                # v = v * size_penalty
                # edge_loss = (2 * cw - w1 - w2 + 2 * ch - h1 - h2) / (4 * (w2 + h2))
                # return iou - (rho2 / c2 + alpha * v + 0.3 * edge_loss)
...
class Metric(SimpleClass):
    ...
    @property
    def aps(self):
        """Returns the Average Precision (AP) for small objects."""

        return self.all_ap[:, 0] if len(self.all_ap) else []

    @property
    def apm(self):
        """Returns the Average Precision (AP) for medium objects."""

        return self.all_ap[:, 1] if len(self.all_ap) else []

    @property
    def api(self):
        """Returns the Average Precision (AP) for large objects."""
    ...
    def mean_results(self):
        ...
        # return [self.mp, self.mr, self.map50, self.aps.mean(), self.apm.mean(), self.api.mean(), self.map]
...
class DetMetrics(SimpleClass):
    ...
    def keys(self):
        ...
        # return ["metrics/precision(B)", "metrics/recall(B)", "metrics/mAP50(B)", "metrics/APs(B)", "metrics/APm(B)","metrics/API(B)", "metrics/mAP50-95(B)"]
