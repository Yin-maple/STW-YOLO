...
class nDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3),ChannelAttention(c2),
        ...
        self.sfe = nn.ModuleList(
            nn.Sequential(
                Conv(ch[i - 1], ch[i], 1),  # 通道数调整
                nn.Upsample(scale_factor=0.5, mode='bilinear', align_corners=False)  # 修正下采样
            ) if i > 0 else nn.Identity() for i in range(self.nl)
        )
