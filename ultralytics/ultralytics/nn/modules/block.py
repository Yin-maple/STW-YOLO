...
from ultralytics.nn.pool import StripPooling
...
class SPSPPF(nn.Module):
    ...
    def __init__(self, c1, c2, k=5):
        ...
        self.m = StripPooling(c_,(20, 12),norm_layer)
...
class SPPFCSPC(nn.Module):
    def __init__(self, c1, c2,k=5, g=1, e=0.5, ):
...
class TaC2f(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, scale=0.1, map_reduce=8):
        super(TaC2f, self).__init__()
        self.scale = scale
        self.out_channels = out_planes
...
class ConvD(nn.Module):
    ...
    def __init__(self, c1, c2, k, s, p=0, g=1, d=1, act=False):
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, p, groups=g, dilation=d)
        self.bn = nn.BatchNorm2d(c2)
        self.act = self.default_act if act is True else None
    def forward(self, x):
        """
        ...
        return self.act(self.bn(self.conv(x))) if self.act is not None else self.bn(self.conv(x))

class MC_FEM(ConvD):
    def __init__(self, c1, c2, shortcut, scale=0.1):
        super(MC_FEM, self).__init__(c1, c2, k=1, s=1, p=1, g=1, d=1)
        self.scale = scale
        inter_planes = int(c2 // 8)
        self.level0 = ConvD(c1, inter_planes, 1, 1)
        self.level1 = ConvD( inter_planes, inter_planes*2, k=(1,3), s=1, p=(0, 1),d=1)
        self.level2 = ConvD( inter_planes, inter_planes*2, k=(3,1), s=1, p=(1, 0),d=1)
        self.level3 = ConvD(inter_planes * 2, inter_planes * 2, 3, 1, 5, d=5)
        self.level4 = ConvD(inter_planes, 2 * inter_planes, 3, 1, 1)

        self.cv2 = ConvD(inter_planes * 6, c2, 1, 1)
        self.relu = nn.ReLU(inplace=False)
        self.shortcut = ConvD(c1, c2, 1, 1, 0)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        x_ = self.level0(x)
        x0 = self.level4(x_)
        xl = self.level3(self.level1(x_))
        xr = self.level3(self.level2(x_))
        out = torch.cat((xl, x0, xr), 1)
        out = self.cv2(out) * self.scale
        return self.relu(self.shortcut(x) + out) if self.add else self.relu(out)

class C2MC_FEM(nn.Module):
    """C2f module with an additional attn module."""
    ...
    def __init__(self, c1, c2, n=2, shortcut=True, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(MC_FEM(self.c, self.c, shortcut) for _ in range(n))
...
class BottleneckCPCA(nn.Module):
    """Standard bottleneck."""
...
class C2fCPCA(C2f):
    ...
    def __init__(self, c1, c2, n=1, cpca=False, e=0.5, g=1, shortcut=True):
        """
        Initialize C3k2 module.
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if cpca else BottleneckCPCA(self.c, self.c, shortcut) for _ in range(2)
        )