...
class C3k_CSP(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        # self.m = nn.Sequential(*(SwinTransformerBlock(c_, c_, c_ // 32, n) for _ in range(n)))
...
class C3k_ST(nn.Module):
    ...
    #     c_ = int(c2 * e)  # hidden channels
    #     self.cv1 = Conv(c1, c_, 1, 1)
    #     self.cv2 = Conv((1 + n) * c_, c_, 1)
    #     self.cv2_1 = nn.Conv2d(c1, c_, 1, 1, bias=False)
    #     self.cv3 = nn.Conv2d(c_, c_, 1, 1, bias=False)
    #     self.cv4 = Conv(3 * c_, c2, 1, 1)
    #     self.bn = nn.BatchNorm2d(3 * c_)  # applied to cat(cv2, cv3)
    #     self.act = nn.SiLU()
    #     self.m = nn.ModuleList(SwinTransformerBlock(c_, c_, c_ // 32, n) for _ in range(n))
    ...
    #     y.extend(m(y[-1]) for m in self.m)
    #     y = self.cv2(torch.cat(y, 1))
    #     y1 = self.cv3(self.m2(self.cv1(x)))
    #     y2 = self.cv2_1(x)
    #     return self.cv4(self.act(self.bn(torch.cat((y,y1, y2), 1))))
...
class SFC2f3(nn.Module):
        ...
        Examples:
            >>> m = SFC2f3(512, 512, n=1, a2=True, area=1)
            >>> x = torch.randn(1, 512, 32, 32)
            >>> output = m(x)
            >>> print(output.shape)
            torch.Size([1, 512, 32, 32])
